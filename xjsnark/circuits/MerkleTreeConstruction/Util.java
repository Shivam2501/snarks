...
public class Util {
  ...
  public static UnsignedInteger[] sha2(Leaf input) {

    if (input.node.length != 16) {
    ...
    for (int j = 0; j < 16; j++) {
      words[j].assign(input.node[j], 32);
    }
  ...
  public static Leaf concat(UnsignedInteger[] a1, UnsignedInteger[] a2) {

    Leaf res = new Leaf();
    for (int i = 0; i < 8; i++) {
      res.node[i].assign(a1[i], 32);
      res.node[i + 8].assign(a2[i], 32);
    }
    return res;
  }
}
