...
struct Node sha2(uint32_t* preimage){
    struct Node H;
...
void compute(struct In *input, struct Out *output) {
    ...
    for (j = 0; j < 8; j++) {
        currentNode.array[j] = input->leaf.array[j];
    }

    for (i = 0; i < HEIGHT; i++) {
        for (j = 0; j < 16; j++) {
            if ((input->directionSelector >> i) & 1)
                inputToNextHash[j] = j >= 8 ? currentNode.array[j - 8] : input->proof[i].array[j];
            else
                inputToNextHash[j] = j < 8 ? currentNode.array[j] : input->proof[i].array[j - 8];
        }
        currentNode = sha2(inputToNextHash);
    }

    for (j = 0; j < 8; j++) {
        output->root[j] = currentNode.array[j];
    }
}